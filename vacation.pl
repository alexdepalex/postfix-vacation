...
#             Properly handle failed queries to post_vacation_notification.
#             Fixed log reporting.
#
...
# post_vacation_notification_pkey" in your system logs. This is expected
...
our $db_host = 'hlnlsql02-int';

# connection details
our $db_username = 'alex';
our $db_password = 'checker2';
our $db_name     = 'hl_config';
...
sub already_notified {
    ...
    my $query = qq{INSERT into post_vacation_notification (on_vacation,notified) values (?,?)};
...
# fine. All other error conditions are not fine, however.
        if ($e !~ /(?:_pkey|^Duplicate entry)/) {
            $logger->error("Failed to insert into post_vacation_notification table (to:$to from:$from error:'$e' query:'$query')");
            # Let's play safe and notify anyway
            return 1;
        }
        if ($interval) {
            $logger->error("Checking interval $interval");
            $query = qq{SELECT NOW()-notified_at FROM post_vacation_notification WHERE on_vacation=? AND notified=?};
            ...
            if ($int > $interval) {
                ...
                $query = qq{UPDATE post_vacation_notification SET notified_at=NOW() WHERE on_vacation=? AND notified=?};
...
sub find_real_address {
    ...
    my $realemail = '';
    my $query = qq{SELECT email FROM post_vacation WHERE email=? AND active=$db_true};
    my $stm = $dbh->prepare($query) or panic_prepare($query);
    $stm->execute($email) or panic_execute($query,"email='$email'");
    my $rv = $stm->rows;
    $logger->debug("Nr rows $rv email $email");
...
# Recipient has vacation
    if ($rv == 1) {
        ...
        $query = qq{SELECT goto FROM post_alias WHERE address=? AND (goto LIKE ? OR goto LIKE ? OR goto LIKE ? OR goto = ?)};
        $stm = $dbh->prepare($query) or panic_prepare($query);
        ...
        if ($rv == 1) { 
            ...
            if ($alias =~ /,/) {
                for (split(/\s*,\s*/, lc($alias))) {
                    ...
                    $query = qq{SELECT email FROM post_vacation WHERE email=? AND active=$db_true};
                    $stm = $dbh->prepare($query) or panic_prepare($query);
...
sub send_vacation_email {
    ...
    my $query = qq{SELECT subject,body FROM post_vacation WHERE email=?};
    my $stm = $dbh->prepare($query) or panic_prepare($query);
    $stm->execute($email) or panic_execute($query,"email='$email'");
    my $rv = $stm->rows;
    if ($rv == 1) {
